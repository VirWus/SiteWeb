...
if($_POST){
  ...
  if(!empty($link)){
$query = mysqli_query($connect,"UPDATE support set nom='$nom',resumer='$resumer',type='$type',lien='$link',module='$module' where id='$id' ");
  }else{
    $ext =  $pathparts["extension"];
if($ext == "jpg" || $ext == "png" || $ext == "pdf" || $ext == "doc" || $ext == "docx" || $ext == "xls") {
  move_uploaded_file($_FILES["fileto"]["tmp_name"], $file);
$file = base64_encode($file);
 $query = mysqli_query($connect,"UPDATE support set nom='$nom',resumer='$resumer',type='$type',lien='$file',module='$module' where id='$id' ");
}else{
